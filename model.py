...
class LabelPropagation(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, g, labels, mask=None, post_step=lambda y: y.clamp_(0., 1.), num_layers=50, EI=1, confidence=1 ):
        with g.local_scope():
...
class CorrectAndSmooth(nn.Module):
    ...
    def correct(self, g, y_soft, y_true, mask, num_layers=50, y_soft2=None):
        ...
        with g.local_scope():
            ...
            if self.autoscale:
                smoothed_error = self.prop1(g, error, post_step=lambda x: x.clamp_(-1., 1.), num_layers=num_layers)
                ...
                smoothed_error = self.prop1(g, error, post_step=fix_input, num_layers=num_layers)
        ...
        with g.local_scope():
            ...
            if use_labels:
                ...
                for i in range(0,len(row),200000):
                    summed_lab = y_soft[row[i:i+200000],:] + y_soft[col[i:i+200000],:]
            ...
            return self.prop2(g, y_soft, num_layers=num_layers, EI=EI, confidence=confidence )

    def lightgbm(self, g, features, y_true, mask):
        clf = lgb.LGBMClassifier(num_leaves=50, learning_rate=0.01, n_estimators=300, feature_fraction=0.4, bagging_fraction=0.01)
    ...
    def linear(self, g,text_features, features, y_true, mask, text_model):
        text_features = (text_features - text_features.mean(0))/text_features.std()

        features = torch.cat([features, text_features], dim=1)
        aggregator = MLPLinear2(features.shape[1], text_features.shape[1])

        optimizer = torch.optim.Adam(aggregator.parameters() , lr=0.005)
